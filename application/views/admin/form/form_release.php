...
<div class="modal-header">
  <h5 class="modal-title">Form Release Order</h5>
        ...
        <div class="form-group">
          <div class="row">
            <div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
              ...
              <div class="form-group">
                <label for="exampleInputEmail1">Delv Date</label>
                <input type="date" id="delv_date" name="delv_date" class="form-control col-11" required value="<?=gmdate('Y-m-d', time() + 60 * 60 * 7);?>" min='<?=gmdate('Y-m-d', time() + 60 * 60 * 7);?>'>
              </div>
              <div class="form-group"> 
                    <label for="exampleInputEmail1">Supplier Code(Status->Open)</label>
                    <select class="form-control col-10" name="supplier_code" id="supplier_code"  required="required">
                       <option value=""></option>
...
function statusupload(table3){
  myVar = setTimeout(function(){
        $.ajax({
            ...
            success: function(data){
                ...
                $('#hasil').text("Release order  "+data.success+" success "+data.failed+" failed from "+data.total+" rows");
                ...
                if(parseInt(persen)==100){
                    $("#errormsg").html("<span class='text-success text-bold'> Release order success!!</code>"); 
                    x= 0; 
                    clearTimeout(myVar); 
                      setTimeout(function(){
  ...
  $('#submit').submit(function(e){
      ...
      e.preventDefault();
           $.ajax({
               url:'<?=base_url('planning/release_order?api='.$this->id_t);?>',
               type:"post",
